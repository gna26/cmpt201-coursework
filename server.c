...
it writes it using tcp protocol
2. What data is the client sending to the server?
It is iterating through an array of strings and sending them
Understanding the Server:
1. Explain the argument that the `run_acceptor` thread is passed as an argument.
It creates a starting thread that uses run_acceptor as an argument. This thread
accepts clients
...
main waits until there are enough messages received to stop
4. How are threads used in this sample code?
Threads are used to create multiple clients and a server that accepts them.

Sockets are made non blocking because the threads run independently. The client
threads are non blocking so they can run concurrently.
*/
